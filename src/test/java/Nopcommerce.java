...
//		Assert.assertTrue(promotionNavLink.getAttribute("class").contains("active"));
		...
		Assert.assertTrue(pageHeadingTxt.contains("Add a new discount"));
		 ...
		 cardBodyEl = driver.findElement(By.cssSelector("#discount-cards .card-body"));
		...
		WebElement discountNameTxtField = driver.findElement(By.id("Name"));
		 currentTime = Instant.now();
		 currentTimeValue = String.valueOf(currentTime);
		String discountName = "dicount_" + currentTimeValue;
		discountNameTxtField.sendKeys(discountName);
		String discountNameValue = discountNameTxtField.getAttribute("value");
		Assert.assertEquals(discountNameValue, discountName);
		...
		discountAmountTxtField.sendKeys(discount);
		String discountValue = discountAmountTxtField.getAttribute("value");
		Assert.assertTrue(discountValue.contains(discount));
		
		WebElement startDateInput = driver.findElement(By.id("StartDateUtc"));
		String startDate ="12/31/2021 12:00:00 AM";
		...
		String endDate ="02/28/2022 12:00:00 AM";
		...
		new WebDriverWait(driver, Duration.ofSeconds(3))
		.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("div.alert")));
		closeAlertEl = driver.findElement(By.cssSelector("div.alert button"));
		closeAlertEl.click();
		
		discountItemsInTableEl = driver.findElement(By.id("discounts-grid_info"));
		wait.until(ExpectedConditions.textToBePresentInElement(discountItemsInTableEl, "of"));
		int discountItemsAfterInsert = Integer.parseInt(discountItemsInTableEl.getText().split(" ")[2]) + 1;
//		 Assert.assertEquals(discountItemsAfterInsert, discountItemsInTable);
		
		 WebElement searchBodySection = driver.findElement(By.cssSelector(".content .card-search .search-row"));
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("ajaxBusy")));

		if (!searchBodySection.getAttribute("class").contains("opened")) {
			 searchBodySection.click();
		}
		WebElement discountNameInSearchTxtField = driver.findElement(By.id("SearchDiscountName"));
		discountNameInSearchTxtField.sendKeys(discountName);
		discountNameValue = discountNameInSearchTxtField.getAttribute("value");
		Assert.assertEquals(discountNameValue, discountName);
		
		WebElement discountTypeInSearchEl = driver.findElement(By.id("SearchDiscountTypeId"));
		Select discountTypeInSearchSelect = new Select(discountTypeInSearchEl);
		discountTypeInSearchSelect.selectByValue("2");
		Assert.assertEquals(discountTypeInSearchSelect.getFirstSelectedOption().getText(), "Assigned to products");

		WebElement searchDiscountBtn = driver.findElement(By.id("search-discounts"));
		searchDiscountBtn.click();
		
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("ajaxBusy")));

		 WebElement discountNameAddedCell = driver.findElement(By.cssSelector("table#discounts-grid tbody tr td:nth-child(1)"));
		 WebElement discountTypeAddedCell = driver.findElement(By.cssSelector("table#discounts-grid tbody tr td:nth-child(2)"));
		 WebElement discountAddedCell = driver.findElement(By.cssSelector("table#discounts-grid tbody tr td:nth-child(3)"));
		 WebElement startDateAddedCell = driver.findElement(By.cssSelector("table#discounts-grid tbody tr td:nth-child(4)"));
		 WebElement endDatetAddedCell = driver.findElement(By.cssSelector("table#discounts-grid tbody tr td:nth-child(5)"));
		 WebElement editDiscountBtn = driver.findElement(By.cssSelector("table#discounts-grid tbody tr td:nth-child(7) a.btn"));
		 
		 Assert.assertEquals(discountNameAddedCell.getText(),discountName);
		 Assert.assertEquals(discountTypeAddedCell.getText(),"Assigned to products");
		 Assert.assertEquals(discountAddedCell.getText(),discount +" USD");
		 Assert.assertEquals(startDateAddedCell.getText(),startDate);
		 Assert.assertEquals(endDatetAddedCell.getText(),endDate);

		 editDiscountBtn.click();
		 wait.until(ExpectedConditions.invisibilityOfElementLocated(By.id("ajaxBusy")));
		 
		Assert.assertTrue(driver.getCurrentUrl().contains("/Admin/Discount/Edit"));
		pageHeadingTxt = driver.findElement(By.cssSelector("#discount-form h1")).getText();
		Assert.assertTrue(pageHeadingTxt.contains(productName));
		
		cardBodyEl = driver.findElement(By.cssSelector("#discount-applied-to-products .card-body"));
		WebElement appliedToProductsSection = driver.findElement(By.id("discount-applied-to-products"));
		if (cardBodyEl.getCssValue("display").equals("none")) {
			appliedToProductsSection.click();
		}
		
		WebElement tableEmptyCell = driver.findElement(By.cssSelector("#products-grid tbody tr td"));
		WebElement productsForDiscountApplied = driver.findElement(By.id("#products-grid_info"));
		Assert.assertEquals(tableEmptyCell.getText(), "No data available in table");
		Assert.assertEquals(productsForDiscountApplied.getText(), "No records");
		
		// Store the current window handle
		String winHandleBefore = driver.getWindowHandle();
		
		WebElement addDiscountToProductBtn = driver.findElement(By.id("btnAddNewProduct"));
		addDiscountToProductBtn.click();
		
		// Switch to new window opened
		for(String winHandle : driver.getWindowHandles()){
		    driver.switchTo().window(winHandle);
		}
		WebElement productNameForDiscountTxtField = driver.findElement(By.id("SearchProductName"));
		productNameForDiscountTxtField.sendKeys(productName);
		 productNameValue = productNameForDiscountTxtField.getAttribute("value");
		Assert.assertEquals(productNameValue, productName);
		
		
		WebElement searchForDiscountBtn = driver.findElement(By.id("search-products"));
		searchForDiscountBtn.click();

		WebElement productCheckbox = driver.findElement(By.cssSelector("#products-grid tbody tr td:first-child input"));
		productCheckbox.click();

		Assert.assertTrue(taxExemptCheckbox.isSelected());
		
		WebElement saveForDiscountBtn = driver.findElement(By.id("[name='save']"));
		saveForDiscountBtn.click();
		
		// Switch back to original browser (first window)
		driver.switchTo().window(winHandleBefore);
		
		WebElement productApplidToDiscountCell = driver.findElement(By.cssSelector("#products-grid tbody tr td:first-child"));
		Assert.assertEquals(productApplidToDiscountCell.getText(), productName);

		WebElement productsAppliedTodiscount = driver.findElement(By.id("#products-grid_info"));
		int productsAppliedTodiscountNumber =Integer.parseInt(productsAppliedTodiscount.getText().split(" ")[2]);
		Assert.assertEquals(productsAppliedTodiscountNumber, 1);
